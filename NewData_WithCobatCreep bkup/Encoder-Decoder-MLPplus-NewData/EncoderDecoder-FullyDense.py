...
def scale_data(df, input_columns, output_columns, 
    ...
    if apply_pt:
        if pt_method == 'yeo-johnson':
            pt_input = PowerTransformer(method='yeo-johnson')
            pt_output = PowerTransformer(method='yeo-johnson')
        elif pt_method == 'box-cox':
            pt_input = PowerTransformer(method='box-cox')
            pt_output = PowerTransformer(method='box-cox')
        else:
            raise ValueError(f"Unknown PowerTransformer method: {pt_method}. Choose 'yeo-johnson' or 'box-cox'.")
        
        # Apply Power Transformer to input columns
        for idx in input_columns:
            transformed_data[idx] = pt_input.fit_transform(df[[idx]]).ravel()
        
        # Apply Power Transformer to output columns
        for idx in output_columns:
            transformed_data[idx] = pt_output.fit_transform(df[[idx]]).ravel()
    ...
    if apply_qt:
        ...
        # Apply Quantile Transformer to input columns
        for idx in input_columns:
            transformed_data[idx] = qt_input.fit_transform(transformed_data[[idx]]).ravel()
        
        # Apply Quantile Transformer to output columns
        for idx in output_columns:
            transformed_data[idx] = qt_output.fit_transform(transformed_data[[idx]]).ravel()
    ...
    if apply_sc:
        ...
        if apply_pt:
            joblib.dump(pt_input, 'scales/input_power_transformer.save')
            joblib.dump(pt_output, 'scales/output_power_transformer.save')
        if apply_qt:
            joblib.dump(qt_input, 'scales/input_quantile_transformer.save')
            joblib.dump(qt_output, 'scales/output_quantile_transformer.save')
    ...
    return inputs_scaled, outputs_scaled, input_scaler, output_scaler, transformed_data, pt_input if apply_pt else None, pt_output if apply_pt else None, qt_input if apply_qt else None, qt_output if apply_qt else None
...
def step_decay_schedule(initial_lr=0.1, decay_factor=0.98, step_size=30):
...
columns_to_keep = [
    'Nb', 'Cr', 'V', 'W', 'Zr', '1500 Min Creep CB [1/s]'   # EQ 1273K Density (g/cc), EQ 1273K THCD (W/mK), YS 1000C PRIOR, 1000 Min Creep NH [1/s], Kou Criteria
]
...
threshold_series = df['1500 Min Creep CB [1/s]']
...
# Call the scale_data function with specific options
inputs_scaled, outputs_scaled, input_scaler, output_scaler, transformed_data, pt_input, pt_output, qt_input, qt_output = scale_data(
    df=df,
    input_columns=input_columns,
    output_columns=output_columns,
    apply_sc=True, scaling_method='minmax',      # Use MinMax scaling
    apply_pt=False, pt_method='yeo-johnson',      # Use Yeo-Johnson method for PowerTransformer
    apply_qt=True, qt_method='uniform',          # Use 'normal' distribution for QuantileTransformer
    apply_log1p=False, 
    apply_sigmoid=False,          # Do not apply sigmoid transformation
    apply_sqrt=False,             # Do not apply square root transformation
    apply_cbrt=False              # Do not apply cube root transformation
)
...
outputs_descaled = descale_data(
    ...
    apply_dsc=True, 
    apply_qt=True, qt=qt_output, 
    apply_pt=False, pt=pt_output,
    apply_log1p=False,
    apply_sigmoid=False
)
...
from MLPplus_Model import create_complex_encoder_with_gbn, create_complex_decoder_with_gbn
...
encoder = create_complex_encoder_with_gbn(input_dim=input_dim, latent_dim=latent_dim, 
                                 num_layers=4, 
                                 neurons_per_layer=[2056, 1024, 512, 256], 
                                 lamb=1e-3, alp=0.2, rate=0.2,
                                 vbatch=64)

# Create a decoder with 6 layers, specified neurons per layer, lamb=1e-5, alpha=0.2, dropout rate 0.2
decoder = create_complex_decoder_with_gbn(output_dim=output_dim, latent_dim=latent_dim, 
                                 num_layers=4, 
...
for idx in range(outputs_scaled.shape[1]):
    ...
    msle = mean_squared_log_error(y_test_original[:, idx], predictions[:, idx])
    log_r2 = r2_score(np.log(y_test_original[:, idx]), np.log(predictions[:, idx]))

    plt.text(0.05, 0.95, f'MSLE: {msle:.3f}', transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
    plt.text(0.05, 0.92, f'r$^2$ : {log_r2 :.3f}', transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
