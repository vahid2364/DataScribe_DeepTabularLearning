...
# Define input and output columns
df = pd.read_csv('../../data/DS_5_Ms_cleaned.csv')

# Define input and output columns
input_columns = [
    'Ni', 'Ti', 'Hf', 'Zr', 'Cu', 'Pd', 'Co', 'Nb'
]
...
# File path to Optuna trials CSV
file_path = '../../Hyperparameter-optimization/Encoder-Decoder-FDN-Optuna-overcomplete-YS/optuna_trials.csv'  # Update with the correct path if needed
...
default_params = {
    ...
    "neurons_per_layer_encoder": [16, 32, 64],
    "neurons_per_layer_decoder": [64, 32, 16]
}
...
if os.path.exists(file_path):
    ...
    # Ensure there are enough trials in the DataFrame
    if len(optunaTrials) > 55:
        params = optunaTrials.iloc[55, :]

        latent_dim = int(params.get("params_latent_dim", default_params["latent_dim"]))
        alp   = params.get("params_alpha", default_params["alpha"])
        lamb  = params.get("params_lambda", default_params["lambda"])
        rate  = params.get("params_drop_out_rate", default_params["drop_out_rate"])
        lr    = params.get("params_learning_rate", default_params["learning_rate"])
        batchSize = int(params.get("params_batch_size", default_params["batch_size"]))

        encoder_num_layers = params.get("params_num_layers_encoder", default_params["num_layers_encoder"])
        decoder_num_layers = params.get("params_num_layers_decoder", default_params["num_layers_decoder"])

        try:
            encoder_neurons = json.loads(params.get("user_attrs_neurons_per_layer_encoder", json.dumps(default_params["neurons_per_layer_encoder"])))
        except json.JSONDecodeError:
            encoder_neurons = default_params["neurons_per_layer_encoder"]

        try:
            decoder_neurons = json.loads(params.get("user_attrs_neurons_per_layer_decoder", json.dumps(default_params["neurons_per_layer_decoder"])))
        except json.JSONDecodeError:
            decoder_neurons = default_params["neurons_per_layer_decoder"]
    else:
        print("Not enough trials available in the CSV. Using default values.")
        latent_dim = default_params["latent_dim"]
        alp = default_params["alpha"]
        lamb = default_params["lambda"]
        rate = default_params["drop_out_rate"]
        lr = default_params["learning_rate"]
        batchSize = default_params["batch_size"]
        encoder_num_layers = default_params["num_layers_encoder"]
        decoder_num_layers = default_params["num_layers_decoder"]
        encoder_neurons = default_params["neurons_per_layer_encoder"]
        decoder_neurons = default_params["neurons_per_layer_decoder"]
else:
...
for idx in range(outputs_scaled.shape[1]):
    ...
    # Calculate GMAE, SMAPE, MASE, and RMSPE
    gmae = np.mean(np.abs(np.log(y_test_original[:, idx]) - np.log(y_test_original[:, idx])))
    smape = np.mean(2 * np.abs(predictions[:, idx] - y_test_original[:, idx]) / (np.abs(predictions[:, idx]) + np.abs(y_test_original[:, idx]))) * 100
    mase = mse / np.mean(np.abs(np.diff(y_test_original[:, idx])))  # Simplified MASE
    rmspe = np.sqrt(np.mean(np.square((y_test_original[:, idx] - predictions[:, idx]) / y_test_original[:, idx])))
    ...
    plt.text(0.05, 0.86, f'GMAE: {gmae:.3g}', transform=plt.gca().transAxes, fontsize=14, verticalalignment='top')
    ...
    #plt.scatter(y_train_original[:, idx], predictions_train[:, idx], c='orange')
