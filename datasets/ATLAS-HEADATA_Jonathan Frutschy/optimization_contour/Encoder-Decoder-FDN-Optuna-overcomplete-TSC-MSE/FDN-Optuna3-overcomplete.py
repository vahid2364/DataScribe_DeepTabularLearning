...
def create_fdn_model(trial):
    ...
    input_dim = 7 #X_train.shape[1]  # Number of input features
...
def load_and_split_data():
    ...
    df = pd.read_csv('../../data/data_LIQUID_variable_temprange9_processed.csv')
    ...
    sns.kdeplot(df[output_columns], fill=True, color='blue', label='KDE')
    plt.savefig('target_feature_distribution.jpg', dpi=300)
    plt.show()
    #pause
...
if __name__ == "__main__":
    ...
    try:
        study = optuna.create_study(
            ...
            directions=["minimize"] * num_objectives  # Minimize for each objective (e.g., R² for each feature)
        )
    ...
    best_model.compile(optimizer=optimizer, loss=Huber(delta=0.03))
