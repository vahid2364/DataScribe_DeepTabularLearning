...
def plot_scatter(y_true, y_pred, output_columns, filename_prefix, log_scale=False, y_train=None, predictions_train=None):
    ...
    for idx in range(y_true.shape[1]):
        ...
        y_test_log_safe = np.where(y_true[:, idx] <= 0, 1e-10, y_true[:, idx])
        predictions_log_safe = np.where(y_pred[:, idx] <= 0, 1e-10, y_pred[:, idx])
...
#        smape = np.mean(2 * np.abs(predictions_log_safe - y_test_log_safe / (np.abs(predictions_log_safe)) + np.abs(y_test_log_safe))) * 100
        ...
        rmspe = np.sqrt(np.mean(np.square((y_true[:, idx] - y_pred[:, idx]) / y_pred[:, idx])))
        ...
        plt.text(0.05, 0.88, f'MSLE: {msle:.3g}', transform=plt.gca().transAxes, fontsize=13, verticalalignment='top')
        plt.text(0.05, 0.85, f'RMSLE: {rmsle:.3g}', transform=plt.gca().transAxes, fontsize=13, verticalalignment='top')
        plt.text(0.05, 0.82, f'log R$^2$ : {log_r2:.3f}', transform=plt.gca().transAxes, fontsize=13, verticalalignment='top')
        plt.text(0.05, 0.78, f'GMAE: {gmae:.3g}', transform=plt.gca().transAxes, fontsize=13, verticalalignment='top')
        plt.text(0.05, 0.75, f'SMAPE: {smape:.2f}%', transform=plt.gca().transAxes, fontsize=13, verticalalignment='top')
        plt.text(0.05, 0.72, f'MASE: {mase:.3g}', transform=plt.gca().transAxes, fontsize=13, verticalalignment='top')
        plt.text(0.05, 0.69, f'RMSPE: {rmspe:.3g}', transform=plt.gca().transAxes, fontsize=13, verticalalignment='top')
