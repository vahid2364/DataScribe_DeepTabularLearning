...
def run_optimization(study_name, num_trials, X_train, X_test, y_train, y_test):
    ...
    def objective(trial):
        ...
        ## Calculate metrics - training data
        #y_pred = model.predict(X_train)
        #test_loss = mean_squared_error(y_train, y_pred) # Minimize
        #r2 = r2_score(y_train, y_pred)  # Maximize

        # Calculate metrics - training data
        y_pred = model.predict(X_test)
        test_loss = mean_squared_error(y_test, y_pred) # Minimize
        ...
        trial.set_user_attr('model_summary', model.summary())
...
def load_and_split_data():
    ...
    df = pd.read_csv('../../../data/HTMDEC_MasterTable_Iterations_v5_processed.csv')
...
def save_study_results(study, filename='results/optuna_trials.csv'):
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig('results/optimization_contour_R2.jpg')
    plt.show()
    ...
    neurons_df.to_csv('results/neurons_configurations_with_latent_dim.csv', index=False)
    ...
    if 'val_loss' in history.history:
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig('results/plots/loss.jpg', dpi=300, transparent=True)
        plt.show()
    ...
    #plt.grid(True)
    plt.savefig('results/plots/parity-plot.png',dpi=300)
    plt.show()
...
# %% Repeats training 5 to 10 times (you can change n_runs).
    ...
    for run in range(1, n_runs + 1):
        ...
        callbacks = [
            ...
            ModelCheckpoint(filepath=f'results/weights/bestmodel_weights_run{run}.keras', monitor='val_loss', save_best_only=True, mode='min')
        ]
        ...
        plt.savefig(f'results/plots/loss_run{run}.png')
        plt.close()
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'results/plots/parity_run{run}.png')
        plt.close()
        
        # Save loss history for this run
        history_df = pd.DataFrame(history.history)
        history_df.to_csv(f'results/plots/loss_history_run{run}.csv', index=False)
    ...
    results_df.to_csv("results/plots/run_metrics_summary.csv", index=False)
    
    # Summary statistics
    summary = results_df.describe().loc[['min', 'max', 'mean']]
    summary.to_csv("results/plots/run_metrics_summary_stats.csv")
    print("\n📊 Summary of Runs:")
    print(summary)
    ...
    for run in range(1, n_runs + 1):
        df = pd.read_csv(f'results/plots/loss_history_run{run}.csv')
        df['Run'] = run
        df['Epoch'] = df.index
        all_history.append(df)
    
    combined_loss_history = pd.concat(all_history, ignore_index=True)
    combined_loss_history.to_csv("results/plots/all_loss_histories_combined.csv", index=False)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/plots/combined_loss_with_band.png", dpi=300)
    plt.show()
