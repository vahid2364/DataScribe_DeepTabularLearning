...
#     if x.shape[1] > 1:  # Only apply pooling if the dimension > 1
...
#     x = Conv1D(filters=filters * 4, kernel_size=kernel_size, activation='relu', padding='same')(x)
#     x = BatchNormalization()(x)
#     if x.shape[1] > 1:  # Only apply pooling if the dimension > 1
#         x = MaxPooling1D(pool_size=2)(x)
#     x = Dropout(dropout_rate)(x)
...
#     x = Conv1D(filters=filters * 16, kernel_size=kernel_size, activation='relu', padding='same')(x)
#     x = BatchNormalization()(x)
#     x = Dropout(dropout_rate)(x)

#     x = Conv1D(filters=filters * 8, kernel_size=kernel_size, activation='relu', padding='same')(x)
#     x = BatchNormalization()(x)
#     x = Dropout(dropout_rate)(x)
...
# 1D-CNN Encoder (Now starts with larger filters and decreases)
def create_cnn_encoder(input_dim, latent_dim, filters=64, kernel_size=3, dropout_rate=0.2):
    input_layer = Input(shape=(input_dim, 1))  # Input for 1D-CNN requires (input_dim, 1) shape
    
    # Convolutional Layers (Filters decrease symmetrically)
    x = Conv1D(filters=filters * 4, kernel_size=kernel_size, activation='relu', padding='same')(input_layer)
    x = BatchNormalization()(x)
    if x.shape[1] > 1:  # Only apply pooling if the dimension > 1
        x = MaxPooling1D(pool_size=2)(x)
    x = Dropout(dropout_rate)(x)
...
# 1D-CNN Decoder (Now starts with smaller filters and increases)
def create_cnn_decoder(output_dim, latent_dim, filters=64, kernel_size=3, dropout_rate=0.2):
    latent_inputs = Input(shape=(latent_dim,))
    ...
    # Convolutional layers for decoding (Filters increase symmetrically)
    x = Conv1D(filters=filters, kernel_size=kernel_size, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Dropout(dropout_rate)(x)
