...
plt.legend()
plt.xlabel('Data')
plt.savefig('images/Kdensity-OutputFeatures-LogScaled.jpg')
plt.show()
...
def evaluate_xgboost_complexity(model, model_name='xgb_model.pkl'):
    booster = model.get_booster()

    # 1. Number of trees
    n_trees = len(booster.get_dump())

    # 2. Maximum tree depth (approximate from dump)
    depths = [tree.count('\t') for tree in booster.get_dump()]
    max_depth = max(depths)

    # 3. Total number of nodes
    df_trees = booster.trees_to_dataframe()
    total_nodes = df_trees.shape[0]

    # 4. Model size in MB
    joblib.dump(model, model_name)
    size_mb = os.path.getsize(model_name) / 1e6

    # 5. Number of unique features used
    used_features = df_trees['Feature'].unique()
    n_features_used = len([f for f in used_features if f != 'Leaf'])

    return {
        'Number of Trees': n_trees,
        'Estimated Max Depth': max_depth,
        'Total Nodes/Splits': total_nodes,
...
# Create the model
model = xgboost_model(n_estimators=50, max_depth=10, learning_rate=9.58e-2)
...
complexity = evaluate_xgboost_complexity(model)
print(complexity)
