...
def plot_parity(y_true, y_pred, output_columns, filename_prefix, log_scale=False):
    ...
    for idx in range(y_true.shape[1]):
        ...
        if log_scale:
            ...
            y_test_log_safe = np.where(y_true[:, idx] <= 0, 1e-30, y_true[:, idx])
            predictions_log_safe = np.where(y_pred[:, idx] <= 0, 1e-30, y_pred[:, idx])
            ...
            plt.text(0.05, 0.95, f'MSLE: {msle:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            plt.text(0.05, 0.91, f'RMSLE: {rmsle:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            plt.text(0.05, 0.87, f'log R$^2$ : {log_r2:.3f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            plt.text(0.05, 0.82, f'GMAE: {gmae:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            plt.text(0.05, 0.78, f'SMAPE: {smape:.2f}%', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            plt.text(0.05, 0.74, f'MASE: {mase:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            plt.text(0.05, 0.70, f'RMSPE: {rmspe:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            ...
            plt.text(0.05, 0.95, f'MSE: {mse:.3f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
            plt.text(0.05, 0.92, f'r$^2$ : {r2 :.3f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        ...
        # Save the plot
        plt.savefig(f'{filename_prefix}/scatterplot_{idx}_XGBoost.jpg')
        plt.show()
