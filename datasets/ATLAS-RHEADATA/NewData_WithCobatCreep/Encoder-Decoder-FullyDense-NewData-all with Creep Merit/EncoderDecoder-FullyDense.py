...
#    'Nb', 'Cr', 'V', 'W', 'Zr', 'YS 1000C PRIOR', 'EQ 1273K THCD (W/mK)', 'EQ 1273K Density (g/cc)', 'Kou Criteria', '1300 Min Creep CB [1/s]',   # EQ 1273K Density (g/cc), EQ 1273K THCD (W/mK), YS 1000C PRIOR, 1000 Min Creep NH [1/s], Kou Criteria
    'Nb', 'Cr', 'V', 'W', 'Zr', 'YS 1000C PRIOR', 'EQ 1273K THCD (W/mK)', 'EQ 1273K Density (g/cc)',  'Kou Criteria', 'Creep Merit', '1300 Min Creep CB [1/s]', 
...
# threshold_series = df['500 Min Creep CB [1/s]']
...
# #
# # Series used for threshold
# threshold_series = df['1300 Min Creep CB [1/s]']
...
# Select only feature columns (from column index 5 onward)
...
for col in feature_cols:
    ...
    excess_kurtosis = kurtosis(df[col], nan_policy='omit')  # already excess kurtosis
    complexity = abs(skewness) + abs(excess_kurtosis)
    feature_complexity[col] = complexity

# Convert to DataFrame for easy viewing or sorting
feature_complexity_df = pd.DataFrame.from_dict(feature_complexity, orient='index', columns=['Feature Complexity'])
feature_complexity_df = feature_complexity_df.sort_values('Feature Complexity', ascending=False)
...
encoder = create_complex_encoder(input_dim=input_dim, latent_dim=latent_dim, 
                                 num_layers=3, 
                                 neurons_per_layer=[1024, 512, 256], 
...
FD_EncoderDecoder.count_params()  # in Keras/TensorFlow
...
# Evaluate the autoencoder
loss_train = FD_EncoderDecoder.evaluate(X_train, y_train)
print(f"Train loss: {loss_train}")

loss_test = FD_EncoderDecoder.evaluate(X_test, y_test)
print(f"Test loss: {loss_test}")

# Make predictions
predictions_seenData_scaled = FD_EncoderDecoder.predict(X_train)
...
predictions_seenData = descale_data(predictions_seenData_scaled,
                 input_scaler=input_scaler, output_scaler=output_scaler,
                 apply_dsc=True, 
                 apply_qt=True, qt_inputs=qt_inputs, qt_outputs=qt_outputs, 
                 apply_pt=False, pt_inputs=None, pt_outputs=None, 
...
for idx in range(outputs_scaled.shape[1]):
    ...
    # Annotate train MSE on the plot
    mse = mean_squared_error(y_train[:, idx], predictions_seenData_scaled[:, idx])
    r2 = r2_score(y_train[:, idx], predictions_seenData_scaled[:, idx])
    plt.text(0.75, 0.15, f'MSE: {mse:.3f}', transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
    plt.text(0.75, 0.12, f'r$^2$ : {r2 :.3f}', transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')


    ## Plot predictions vs actual outputs for the first output feature
    plt.scatter(y_test[:, idx], predictions_scaled[:, idx], c='gray')
...
for idx in range(outputs_scaled.shape[1]):
    ...
    try:
        ...
        y_test_log_safe = np.where(y_test_original[:, idx] <= 0, 1e-20, y_test_original[:, idx])
        predictions_log_safe = np.where(predictions[:, idx] <= 0, 1e-20, predictions[:, idx])
        ...
        plt.text(0.03, 0.95, f'MSLE: {msle:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.03, 0.91, f'RMSLE: {rmsle:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.03, 0.87, f'log R$^2$ : {log_r2:.3f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.03, 0.82, f'GMAE: {gmae:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.03, 0.78, f'SMAPE: {smape:.2f}%', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.03, 0.74, f'MASE: {mase:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.03, 0.70, f'RMSPE: {rmspe:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        ...
        plt.text(0.02, 0.95, f'MSE: {mse:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.02, 0.91, f'R$^2$ : {r2:.3f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.02, 0.87, f'GMAE: {gmae:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.02, 0.82, f'SMAPE: {smape:.2f}%', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.02, 0.78, f'MASE: {mase:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
        plt.text(0.02, 0.74, f'RMSPE: {rmspe:.3g}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    ...
    plt.xlabel(f'Actual: {output_columns[idx]}', fontsize=20)
    plt.ylabel('Predicted', fontsize=20)
    plt.tight_layout()

    # Save the figure
    plt.savefig(f'parityplots-original/scatterplot_{idx}_FDN.jpg')
    plt.show()
