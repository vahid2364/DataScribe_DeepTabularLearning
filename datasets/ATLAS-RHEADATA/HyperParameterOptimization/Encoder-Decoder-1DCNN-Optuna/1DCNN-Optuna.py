...
def run_optimization(study_name, num_trials, X_train, X_test, y_train, y_test):
    ...
    def create_cnn_model(trial):
        """
        Function to create and return an CNN model with specified parameters.
        """
        latent_dim = trial.suggest_int('latent_dim', 64, 192, step=32)
        #filters = trial.suggest_int('filters', 32, 128, step=32)
        rate = trial.suggest_float('drop_out_rate', 0.1, 0.3, step=0.1)
        kernel_size = trial.suggest_int('kernel_size', 2, 4, step=1)
        ...
        encoder = create_cnn_encoder(input_dim=input_dim, filters=64, latent_dim=latent_dim, kernel_size=kernel_size, dropout_rate=rate)
        decoder = create_cnn_decoder(output_dim=output_dim, filters=64, latent_dim=latent_dim, kernel_size=kernel_size, dropout_rate=rate)
    ...
    storage = RDBStorage(
        url="sqlite:///distributed-CNN-Optuna.db"
    )
...
if __name__ == "__main__":
    ...
    study_name = 'distributed-CNN-Optuna'
    ...
    plt.savefig('optimization_history-CNN.jpg')
    plt.show()
    ...
    ax.xaxis.label.set_color('black')
    ax.yaxis.label.set_color('black')
    plt.savefig('hyperparameter_importance-CNN.jpg')
    plt.show()  
    
    plt.figure(figsize=(10, 6))
    ax = optuna.visualization.matplotlib.plot_contour(study, params=["latent_dim", "batch_size"])
    plt.tight_layout()
    plt.savefig('optimization_contour_XY-CNN.jpg')
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('optimization_3d_CNN.jpg')
    plt.show()
    ...
    plt.savefig('customized_contour_plot_CNN.jpg')
