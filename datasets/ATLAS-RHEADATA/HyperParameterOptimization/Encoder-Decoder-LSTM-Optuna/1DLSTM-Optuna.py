...
# Import the functions from OneDLSTM_Model.py
from OneDLSTM_Model import create_lstm_encoder, create_lstm_decoder
...
def run_optimization(study_name, num_trials, X_train, X_test, y_train, y_test):
    ...
    def create_LSTM_model(trial):
        """
        Function to create and return an LSTM model with specified parameters.
        """
        ...
        lstm_units = trial.suggest_int('lstm_units', 192, 256, step=32)
        learning_rate = trial.suggest_float('learning_rate', 1e-4, 1e-3, log=True)
        optimizer_name = trial.suggest_categorical('optimizer', ['adam', 'sgd', 'adadelta'])

        # Reshape input for LSTM to (input_dim, 1) to add the channel dimension
        ...
        # Reshape input for LSTM to (input_dim, 1) to add the channel dimension
        LSTM_input_shape = (input_dim, 1)
        
        # Create LSTM encoder and decoder        
        encoder = create_lstm_encoder(input_dim=input_dim, latent_dim=latent_dim, lstm_units=lstm_units, dropout_rate=rate)
        decoder = create_lstm_decoder(output_dim=output_dim, latent_dim=latent_dim, lstm_units=lstm_units, dropout_rate=rate)

        
        # Create the autoencoder model
        autoencoder_input = Input(shape=LSTM_input_shape)  # Input shape should be (input_dim, 1)
        encoded = encoder(autoencoder_input)
        decoded = decoder(encoded)
        OneDLSTM_EncoderDecoder = Model(inputs=autoencoder_input, outputs=decoded)
        ...
        OneDLSTM_EncoderDecoder.compile(optimizer=optimizer, loss='mse')
        return OneDLSTM_EncoderDecoder
    ...
    storage = RDBStorage(
        url="sqlite:///distributed-LSTM-Optuna.db"
    )
...
if __name__ == "__main__":
    ...
    study_name = 'distributed-LSTM-Optuna'
    ...
    plt.tight_layout()
    plt.savefig('optimization_history-LSTM.jpg', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('hyperparameter_importance-LSTM.jpg')
    plt.show()  
    
    plt.figure(figsize=(10, 6))
    ax = optuna.visualization.matplotlib.plot_contour(study, params=["latent_dim", "batch_size"])
    plt.tight_layout()
    plt.savefig('optimization_contour_XY-LSTM.jpg', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig('optimization_3d_LSTM.jpg')
    plt.show()
    ...
    plt.savefig('customized_contour_plot_LSTM.jpg')
