...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 10:57:21 2024
...
def test_residuals(df, x_cols, y_col):
    ...
    print("residuals shape:", residuals.to_numpy().ravel().shape)
    print("X shape:", X.shape)
    
    for i, col in enumerate(output_columns):
        resid_i = residuals[col].to_numpy().ravel()
        bp_test = het_breuschpagan(resid_i, X)
        print(f"{col}: p-value = {bp_test[1]:.4f}")

    # 2.2 Statistical Test: Breusch-Pagan Test for Homoscedasticity
    #bp_test = het_breuschpagan(residuals.to_numpy().ravel(), X)
...
input_columns = df.columns[:5]  # First 5 columns (input features)
...
# Loop over each selected output feature from the predefined list
for output_col in output_columns:
    print(f"\nProcessing for output feature: {output_col}")
    
    # Separate the input features and current output feature
    X = df[input_columns].values  # Inputs (Nb, Cr, V, W, Zr)
    y = df[output_col].values  # Selected output feature

    # Apply PowerTransformer on the input features
    X_transformed = pt.fit_transform(X)
    
    # Apply QuantileTransformer on the selected output feature
    y_transformed = qt.fit_transform(y.reshape(-1, 1))  # Reshape y to 2D array for transformation

    # Now you can proceed with calculations or model training with `X_transformed` and `y_transformed`
    print(f"Transformed inputs (X):\n{X_transformed[:5]}")  # Print first 5 rows as a sample
    print(f"Transformed output (y):\n{y_transformed[:5]}")  # Print first 5 rows as a sample
    
    # Additional processing or model training code can go here
