...
# Load and split the data, allowing dynamic output column selection
def load_and_split_data(output_columns=['Kou Criteria']):
    # Load the CSV file
    df = pd.read_csv('../../input_data/v2/NbCrVWZr_data_stoic_creep_equil_filtered_v2_IQR_filtered.csv')
    
    # Define input columns (assuming these are fixed)
    input_columns = ['Nb', 'Cr', 'V', 'W', 'Zr']
    
    # Process and split the data, passing the dynamic output_columns
...
#    'Nb', 'Cr', 'V', 'W', 'Zr', 'Creep Merit', '25 Min Creep CB [1/s]', '500 Min Creep CB [1/s]', '1300 Min Creep CB [1/s]', '1500 Min Creep CB [1/s]'   # EQ 1273K Density (g/cc), EQ 1273K THCD (W/mK), YS 1000C PRIOR, 1000 Min Creep NH [1/s], Kou Criteria
    'Nb', 'Cr', 'V', 'W', 'Zr', 'Kou Criteria', 'EQ 1273K Density (g/cc)', 'EQ 1273K THCD (W/mK)', 'YS 1000C PRIOR', '1000 Min Creep CB [1/s]', 'Kou Criteria'
]
...
# Loop through each output column and generate the KDE plot (Log Scale)
plt.figure(figsize=(8,6))
for col in output_columns:
    sns.kdeplot(df[col], label=col, fill=True, log_scale=True)
plt.legend()
plt.xlabel('Output Features (Log Scale)')
plt.savefig('images/Kdensity-OutputFeatures-LogScale.jpg')
plt.show()

# %%

# Loop through each output column and generate the KDE plot (Normal Scale)
plt.figure(figsize=(8,6))
for col in output_columns:
...
encoder = create_complex_encoder(input_dim=input_dim, latent_dim=latent_dim, 
                                 ...
                                 lamb=3.69e-6, alp=0.0164, rate=0.1)
...
for run in range(n_runs):
    ...
    FD_EncoderDecoder.save(model_save_path)
...
stat, p_value = friedmanchisquare(*df_all_runs.values)
...
train_loss = FD_EncoderDecoder.evaluate(X_train, y_train)
...
for idx in range(outputs_scaled.shape[1]):
    ...
    plt.text(0.05, 0.95, f'MSE: {mse:.4f}', transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
...
for idx in range(outputs_scaled.shape[1]):
    ...
    try:
        ...
        msle = mean_squared_log_error(y_test_original, predictions)
        log_r2 = r2_score(np.log(y_test_original), np.log(predictions))
        
        # Annotate MSLE and log r² on the plot
        plt.text(0.05, 0.95, f'MSLE: {msle:.4g}', transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
